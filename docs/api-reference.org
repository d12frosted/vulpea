...
result is a faithful map of every registered schema. The include
relationships are read from =vulpea-schema-includes=, the names a schema
now retains from its =:include=. The whole scan is one query plus an
in-memory pass, so it stays sub-second for collections of several
thousand notes.
