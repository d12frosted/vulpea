...
(vulpea-note-modified-at note)   ; Timestamp of the note's last sync
#+end_src
...
the note has no =CREATED= property. =vulpea-note-modified-at= is a
=YYYY-MM-DD HH:MM:SS= timestamp recorded when the note was last synced
into the database; it tracks indexing time rather than the file's own
modification time (for the latter, see =vulpea-db-query-stale-notes=,
which reads the =files= table).
