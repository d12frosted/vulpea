...
#+begin_src emacs-lisp
;; Get all attachment destinations for notes in a file
(vulpea-db-query-attachments-by-path "/path/to/file.org")
;; => ("image.png" "document.pdf" ...)
#+end_src

This is an optimized query that retrieves all attachment link destinations in a single database query, avoiding N+1 queries when processing multiple notes in a file.
...
| =vulpea-db-query-attachments-by-path= | Attachment destinations in file |
