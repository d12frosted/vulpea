...
- =vulpea-db-note-index-filter-functions= is an abnormal hook run for
  each note in =vulpea-db-update-file= before it is inserted. A handler
  receives the =vulpea-note= about to be indexed and returns non-nil to
  allow it or nil to skip it; a note is indexed only when every handler
  allows it. This is the general extension point the validation action
  builds on.
