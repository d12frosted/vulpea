...
Ids double as interactive handles in both commands. Every candidate carries its note id as an invisible, matchable suffix (=vulpea-select-match-ids=, default on), so typing or pasting an id narrows completion to that note - =vulpea-find= visits it, =vulpea-insert= links it. As a backstop, a fully typed id that still comes back unmatched is resolved before the create path (=vulpea-resolve-id-input=, default on); this runs only when nothing in the completion list matched, so titles keep priority. The =vulpea-insert= link description comes from =vulpea-insert-default-description-fn= (the note title by default; a region still wins) - override it to derive the description from the id or anything else. See [[file:user-guide.org::*Selecting by Id][Selecting by Id]] in the user guide.
