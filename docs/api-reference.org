...
Both functions take a =vulpea-note= and return a string, so you can assemble any layout from the note's slots. Because the whole =vulpea-note= is available, you can combine fields - outline path, tags, timestamps, even a precomputed backlink count - into a single aligned row. For example, a fixed-width row with directory, hierarchy, tags and backlink count:

#+begin_src emacs-lisp
(defun my-vulpea-describe (counts)
  "Return a describe function closing over a backlink COUNTS table."
  (lambda (note)
