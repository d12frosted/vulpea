...
The reaction defaults to =auto=: on when extraction output actually
depends on dir-locals (=vulpea-db-parse-method= is =find-file=, or a
registered extractor declares =:reads-dir-locals t=).  Re-indexing
the whole subtree over-approximates shadowing by deeper dir-locals
files - wasted work, but never a stale note.  The startup scan and
=vulpea-db-sync-full-scan= diff the stored hashes too, so edits made
while Emacs was closed are picked up (something live watchers can
never do).  Dir-locals classes defined from elisp
(=dir-locals-set-class-variables=) have no file to watch: after
changing those, run a force scan manually.
