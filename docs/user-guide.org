...
If two notes would end up with exactly the same candidate (same title, aliases and tags), Vulpea keeps both selectable by appending a short note id to each - so a duplicate never silently shadows another note. That is only a safety net; to make same-named notes genuinely easy to tell apart, give them distinguishing text with the variables below (or fix the duplication with =vulpea-db-query-title-collisions=).
