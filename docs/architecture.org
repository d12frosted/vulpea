...
*** Today
Vulpea v2 has no org-roam dependency. The compatibility package was never written: nobody asked for it, and the two coexist on separate databases without it.
...
(vulpea-db-register-extractor
  (make-vulpea-extractor
   :name 'citations
   :version 1
   :requires-ast t  ; reads the AST, which is opt-in
