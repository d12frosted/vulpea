...
- Fast lookups by ID
- Full rebuild is fast: [[https://github.com/meedstrom/org-node#benchmarks-on-my-machine][org-node's README]] measures 2 seconds for its author's 3000 nodes
...
- Picks up external changes (git pull, Syncthing, etc.) through fswatch, or by polling every few seconds without it
...
- *Byte compiler is your friend* - Accessors are functions, so typos and wrong arity are flagged at compile time (vs raw SQL which only fails at runtime)
...
I have not benchmarked org-roam or org-node myself, so this section only has numbers for vulpea (measured, see =bench/=). Qualitatively: org-node's speed comes from the regex parser and in-memory storage, org-roam pays for a full org-element parse on every save, and vulpea's initial index runs in the background, so its duration does not block your workflow.

With async extraction enabled, vulpea gets the property that used to be unique to org-node - parsing happens outside the main Emacs process - while keeping the native parser. The numbers for a single large save (the freeze you actually feel with autosync on), from [[file:../bench/README.md][bench/README.md]]:
...
For plain note-taking, any of the three will do. For building on top of your notes, vulpea is the one designed for it.
