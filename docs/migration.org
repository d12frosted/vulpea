#+TITLE: Migration Guide: Vulpea v1 to v2
#+AUTHOR: Boris Buliga

Vulpea v2 is a complete rewrite. This guide helps you migrate from v1.

* Overview of Changes

** What's Different

| Aspect       | v1                          | v2                        |
|--------------+-----------------------------+---------------------------|
| Database     | org-roam's database         | Custom database           |
| Sync         | Save hooks (blocking)       | File watchers + queue     |
| Dependencies | Requires org-roam           | Standalone                |
| API          | Built on org-roam functions | Native functions          |

** What's the Same

- Note format (org files with IDs)
- Metadata syntax (description lists)
- Core concepts (tags, links, metadata)

* Before You Migrate

Try v2 in a separate Emacs session first. v1 and v2 can index the same
files, since each keeps its own database:

#+begin_src emacs-lisp
;; v2 uses its own database, separate from org-roam-db-location
(setq vulpea-db-location
      (expand-file-name "vulpea/vulpea.db" user-emacs-directory))
#+end_src

* Configuration Migration

** Directory Configuration

*v1:*
#+begin_src emacs-lisp
;; Used org-roam-directory
#+end_src

*v2:*
#+begin_src emacs-lisp
;; Defaults to org-directory, so often no config needed!
;; Only set if different from org-directory:
(setq vulpea-db-sync-directories '("~/org/"))
#+end_src

Note: =vulpea-default-notes-directory= defaults to the entry of =vulpea-db-sync-directories= holding the current buffer (the first entry outside any of them), so you typically don't need to set it.

** Database Location

*v1:* Used org-roam's database

*v2:*
#+begin_src emacs-lisp
(setq vulpea-db-location
      (expand-file-name "vulpea.db" user-emacs-directory))
#+end_src

** Enabling Sync

*v1:* Relied on org-roam-db-autosync-mode

*v2:*
#+begin_src emacs-lisp
(vulpea-db-autosync-mode +1)
#+end_src

* API Migration

** Query Functions

The query functions keep their names: =vulpea-db-query=,
=vulpea-db-get-by-id=, and the =-by-tags-= and =-by-links-= families
are all still there. The one casualty is =vulpea-db-get-title=; use
=vulpea-db-search-by-title= instead.

** Note Creation

*v1:*
#+begin_src emacs-lisp
(vulpea-create title
               "${slug}.org"  ; file-name was required
               :tags '("project" "work")
               :head "#+created: %<[%Y-%m-%d]>"
               :immediate-finish t)
#+end_src

*v2:*
#+begin_src emacs-lisp
(vulpea-create title
               nil  ; file-name (nil = use template)
               :tags '("project" "work")
               :head "#+created: %<[%Y-%m-%d]>")
#+end_src

Note: v2 always creates notes immediately (no capture buffer). The =:immediate-finish=, =:unnarrowed= and =:capture-properties= keywords are gone, and =file-name= is optional.

** Metadata

The metadata API is largely unchanged:

#+begin_src emacs-lisp
;; Both versions
(vulpea-note-meta-get note "key")
(vulpea-note-meta-get note "key" 'number)
(vulpea-note-meta-get-list note "key")
#+end_src

** Buffer Functions

Buffer functions are unchanged:

- =vulpea-buffer-title-get=
- =vulpea-buffer-title-set=
- =vulpea-buffer-tags-get=
- =vulpea-buffer-tags-set=
- =vulpea-buffer-meta-set=

* Database Migration

v2 uses a different database schema. You cannot reuse the v1/org-roam database.

** Building Fresh Database

#+begin_src emacs-lisp
;; Configure v2
(setq vulpea-db-sync-directories '("~/org/"))
(setq vulpea-db-location "~/.emacs.d/vulpea.db")

;; Build database
(vulpea-db-sync-full-scan)
#+end_src

Your notes don't change - only the database is rebuilt.

* org-roam Compatibility

v2 does not depend on org-roam, but they can coexist:

- Each maintains its own database
- No conflicts between them
- Use either or both for queries

If you want to keep org-roam for some features while using Vulpea for others, both can be loaded simultaneously.

* Breaking Changes Summary

1. *Different database* - rebuilt from your files, nothing to migrate
2. *No org-roam dependency* - org-roam functions are not loaded for you
3. *Async sync* - the database catches up a moment after a save, not during it
4. *=vulpea-create=* - capture keywords are gone and =file-name= is optional (see above)

If something does not migrate cleanly, check [[file:troubleshooting.org][Troubleshooting]], then [[https://github.com/d12frosted/vulpea/issues][open an issue]] with your v1 configuration and the error.
