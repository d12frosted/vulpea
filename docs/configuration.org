...
#+begin_src emacs-lisp
(setq vulpea-db-async-extraction t)  ; Off by default
#+end_src
...
With async extraction, reading, hashing, parsing, and extraction run
in a persistent =emacs --batch= worker; the main thread only blocks
for the database write of the results (a 1MB file: ~50ms instead of
~360ms; a 10MB file: ~0.5s instead of ~3.5s).
