...
The watchers read =vulpea-db-sync-directories= when the mode starts: if you change the variable later, toggle the mode off and on to apply the new value (=M-x vulpea-doctor= flags the mismatch).
