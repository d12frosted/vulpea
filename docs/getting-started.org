...
Vulpea v2 is currently on the =v2-rewrite= branch:
...
Tell Vulpea where your notes live:

#+begin_src emacs-lisp
(require 'vulpea)

;; Directory (or directories) containing your org notes
(setq vulpea-db-sync-directories '("~/org/"))

;; Where to store the database (optional, has sensible default)
...
| Aliases     | =ROAM_ALIASES= property         | Included in completion candidates       |
