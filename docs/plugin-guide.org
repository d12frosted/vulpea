...
- =:id= - note ID (UUID)
- =:level= - heading level; present on heading-level notes only,
  absent for the file-level note
- =:pos= - position in file; present on heading-level notes only
- =:title= - note title
...
The file path is not part of =note-data= - read it from the parse
context via =(vulpea-parse-ctx-path ctx)=.  To tell the file-level
note from heading-level ones, compare =:id= against the context's
file node (see the aggregate example below).
