...
#+begin_src emacs-lisp
;; Check if table exists.  Pass the literal SQL name: emacsql
;; converts dashes in identifiers to underscores, so a table
;; declared as my-table exists as my_table.
(vulpea-db--table-exists-p 'my_table)
