...
Each element is (BEG END TYPE TEXT), ready for `flymake-make-diagnostic'."
  (let* ((ast (org-element-parse-buffer))
         ...
         (datas (cons (vulpea-db--extract-file-node ast path (current-buffer) file-title file-category)
                      (vulpea-db--extract-heading-nodes ast path (current-buffer) file-title file-category)))
         (diagnostics nil))
    (dolist (data datas)
