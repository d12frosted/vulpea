...
;;; Disambiguation Tests (Issue #326)

(ert-deftest vulpea-select-from-disambiguates-identical-candidates ()
  "Notes that render to the same candidate stay individually reachable.

Two notes with the same title, aliases and tags would otherwise
collapse into a single completion candidate, making all but the first
unreachable."
  (let* ((note1 (make-vulpea-note
                 :id "aaaaaaaa-0000-0000-0000-000000000001"
                 :title "Duplicate" :level 0 :tags '("tag")))
         (note2 (make-vulpea-note
    ...
    ;; both notes are present and as distinct candidate strings
    (should (= (length candidates) 2))
    (should (= (length (seq-uniq candidates)) 2))))

(ert-deftest vulpea-select-from-disambiguation-resolves-each-note ()
  "Selecting either of two identical-looking notes returns the right note."
  (let* ((note1 (make-vulpea-note :id "aaaaaaaaaaaa" :title "Dup" :level 0))
         (note2 (make-vulpea-note :id "bbbbbbbbbbbb" :title "Dup" :level 0))
         (notes (list note1 note2))
         (select
          (lambda (fragment)
            (cl-letf (((symbol-function 'completing-read)
                       (lambda (_prompt collection &rest _)
                         (seq-find
                          (lambda (cand) (string-match-p fragment cand))
                          (all-completions "" collection)))))
              (vulpea-select-from "Note" notes)))))
    (should (equal (vulpea-note-id (funcall select "aaaa")) "aaaaaaaaaaaa"))
    (should (equal (vulpea-note-id (funcall select "bbbb")) "bbbbbbbbbbbb"))))

(ert-deftest vulpea-select-from-no-disambiguation-when-unique ()
  "Distinct candidates are left untouched - no id suffix is appended."
  (let* ((note1 (make-vulpea-note :id "id1" :title "Alpha" :level 0))
         (note2 (make-vulpea-note :id "id2" :title "Beta" :level 0))
         (candidates nil))
    (cl-letf (((symbol-function 'completing-read)
               (lambda (_prompt collection &rest _)
                 (setq candidates (all-completions "" collection))
...
(ert-deftest vulpea-select--unique-prefix-length-grows-on-shared-prefix ()
  "The id prefix grows when ids collide at the minimum length."
  (let ((vulpea-select-disambiguator-length 8))
    ;; share first 8 chars, differ at the 9th
    (should (= (vulpea-select--unique-prefix-length
                '("123456780000" "123456781111"))
               9))
    ;; already distinct within the minimum length
    (should (= (vulpea-select--unique-prefix-length
                '("abcdefgh0000" "ijklmnop1111"))
               8))))

(ert-deftest vulpea-select-from-disambiguation-extends-shared-id-prefix ()
  "Disambiguation extends the id prefix when the minimum is not enough."
  (let* ((vulpea-select-disambiguator-length 8)
         (note1 (make-vulpea-note :id "1234567800000000" :title "Same" :level 0))
         (note2 (make-vulpea-note :id "1234567811111111" :title "Same" :level 0))
         (candidates nil))
    (cl-letf (((symbol-function 'completing-read)
               (lambda (_prompt collection &rest _)
                 (setq candidates (all-completions "" collection))
