...
- [[https://github.com/d12frosted/vulpea/issues/326][vulpea#326]] Note selection no longer hides notes that render to the same candidate string. When two or more notes share their entire completion candidate - same =vulpea-select-describe-fn= and =vulpea-select-annotate-fn= output, i.e. same title, aliases and tags - all but the first used to be unreachable: completion UIs collapse identical strings, and the candidate-to-note lookup resolved by string equality and so always returned the first match. =vulpea-select-from= (and therefore =vulpea-find=, =vulpea-insert= and =vulpea-select=) now appends a short, stable note id prefix to each candidate in such a collision group, so every note stays distinct and selectable. Unique candidates are untouched, so the common case is unchanged. The new =vulpea-select-disambiguator-length= controls the minimum length of that id prefix (default 8); the prefix is extended automatically when a longer one is needed to tell the colliding notes apart, so reachability holds regardless of the setting. To control how duplicates are presented (rather than just kept reachable), customize =vulpea-select-describe-fn=/=vulpea-select-annotate-fn= - for example via =vulpea-select-dyncontext-fn= to add a distinguishing suffix only to colliding notes.
