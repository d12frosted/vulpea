...
- [[https://github.com/d12frosted/vulpea/issues/388][vulpea#388]] Autosync now re-indexes affected notes when a =.dir-locals.el= (or =.dir-locals-2.el=) changes. Extraction output can depend on dir-locals (the =find-file= parse method, or an extractor plugin reading them from the note's path), and editing such a file used to leave the database silently stale until a manual force re-index: note contents did not change, so nothing was re-extracted. Both watch backends already delivered these events; vulpea now tracks content hashes of dir-locals files and, on a real change (creation, edit, deletion or rename; a mere touch does not count), force re-indexes all org files under that directory, loudly ("dir-locals changed under X, re-indexing N files..."). The startup scan diffs the stored hashes too, so edits made while Emacs was closed are picked up as well. Gated by the new =vulpea-db-sync-reindex-on-dir-locals-change=: the default =auto= reacts only when extraction actually depends on dir-locals, that is when =vulpea-db-parse-method= is =find-file= or a registered extractor declares the new =:reads-dir-locals t= (same intent-declaration pattern as =:requires-ast=); =t= / =nil= force the reaction on or off. Out of scope: directory classes defined from elisp via =dir-locals-set-class-variables= have no file to watch, changing those still needs a manual force scan.
