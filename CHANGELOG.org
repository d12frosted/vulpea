...
- [[https://github.com/d12frosted/vulpea/issues/326][vulpea#326]] Note selection now resolves the picked candidate to the exact note via the =vulpea-note-id= text property that each candidate already carries, instead of returning the first candidate whose string is =equal=. When two notes render to the same candidate (same title, aliases and tags) and the completion frontend returns the pick with its text properties intact (Vertico, Consult, Ivy and the like), selecting one returns that note rather than always the first. Frontends that strip text properties (e.g. vanilla =completing-read=) fall back to the previous string match, and an unmatched input still yields a virtual note. This is purely about which note you get back - it does not change how candidates are displayed. Keeping same-looking notes visually distinct (or individually visible in UIs that collapse identical rows) remains a presentation choice: customize =vulpea-select-describe-fn= / =vulpea-select-annotate-fn= (e.g. via =vulpea-select-dyncontext-fn=), or resolve the duplication surfaced by =vulpea-db-query-title-collisions=.
