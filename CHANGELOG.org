...
- [[https://github.com/d12frosted/vulpea/issues/359][vulpea#359]] Extractor plugins can now declare how they interact with async extraction. =make-vulpea-extractor= gains =:requires-ast= (default =t=): declaring =nil= states the extract function works purely from note data, which keeps extraction at the fast =element= granularity and keeps files eligible for the worker (the extractor then runs in the main process during apply, against a context whose AST may be nil). Previously any registered extractor silently disabled the entire async pipeline and forced =object= parsing - a config with a simple attachment scanner got neither improvement. Additionally, =:worker-safe t= plus =:worker-lib= (a feature or file defining the function) lets the extractor run inside the worker in =full= mode, writing its tables through the worker's own connection; if the worker cannot resolve the function it transparently falls back to streaming results, so the extractor runs in the main process and nothing is lost. =vulpea-db-worker-rejection-reasons= makes the routing verdict inspectable.
