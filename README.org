...
*** Plugin System
- Parse-once architecture: single org-element parse shared across extractors
- =vulpea-extractor= struct with schema, version, priority, and extraction function
- Automatic schema creation and version tracking
- Priority-based execution order (0-9 reserved for core, 10+ for plugins)
- Custom database tables with foreign key constraints
- Example citation extractor demonstrating full plugin lifecycle
- See [[file:docs/v2-plugin-guide.org][Plugin Guide]] for complete documentation
...
- [X] Plugin System (7 tests passing)
...
Vulpea v2 only indexes notes that have an =ID= property. Notes without IDs are ignored.

**** File-Level Notes

File-level notes require an =ID= property in a property drawer at the beginning of the file:
...
Heading-level notes require an =ID= property in the heading's property drawer:
...
You can exclude notes from indexing using the =VULPEA_IGNORE= property, even if they have an =ID=:
...
This note won't be indexed by Vulpea.
#+end_src

This works for both file-level and heading-level notes.

**** Creating IDs

Use =org-id-get-create= to add IDs to your notes:

#+begin_src emacs-lisp
;; Add ID to current note (file or heading)
(org-id-get-create)

;; Programmatically add ID to a file
(with-current-buffer (find-file-noselect "/path/to/note.org")
  (goto-char (point-min))
  (org-id-get-create)
  (save-buffer))
#+end_src
...
See [[file:docs/v2-plugin-guide.org][Plugin Guide]] for comprehensive documentation on writing plugins.
...
- [[file:docs/v2-plugin-guide.org][Plugin Guide]] - Complete guide for writing custom extractors
...
|-----------+--------+--------|
| Tag query (10k notes) | <50ms | ✅ Implemented, not yet benchmarked |
| Get note by ID | <5ms | ✅ Implemented, not yet benchmarked |
| File update (agenda) | <200ms | ✅ Implemented, not yet benchmarked |
| Batch update (100 files) | <10s | ✅ Implemented, not yet benchmarked |
