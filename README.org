...
|---------+------------------------------------+----------------------------------------------------------------|
| =fd=      | 15× faster directory scanning      | =brew install fd= / =apt install fd-find= / =pacman -S fd=           |
...
Review these settings before relying on autosync—picking the right trade-offs here
...
- =single-temp-buffer=: ⚡ ~1k files/sec. Skips org-mode hooks and per-file keywords.
- =temp-buffer=: ✓ Honors org-mode hooks per file. ~460/sec, degrades to ~56/sec at scale.
- =find-file=: ✓ Respects dir-locals/file-visiting hooks. ~34/sec, degrades to ~16/sec at scale.

If an =org-mode-hook= is expensive (e.g. =org-roam=), wrap it in
=(unless (bound-and-true-p 'vulpea-db--active-parse-method) …)= so it is
...
Vulpea includes a full-featured journal system for daily notes with customizable widgets, calendar integration, and a two-pane interface.
...
(vulpea-journal (encode-time 0 0 12 25 11 2024))
#+end_src

*** Configuration

All journal configuration is in a single template variable:

#+begin_src emacs-lisp
(setq vulpea-journal-default-template
      '(:file-name "journal/%Y%m%d.org"   ; strftime, relative to vulpea-directory
        :title "%Y-%m-%d %A"              ; strftime
        :tags ("journal" "daily")         ; first tag identifies journal notes
        :head "#+created: %<[%Y-%m-%d]>"
        :body "* Morning\n\n* Evening\n"
...
|---------------+----------+--------------------------------------------------------|
| =:file-name=    | yes      | File path (strftime), relative to =vulpea-directory=     |
| =:title=        | yes      | Note title format (strftime)                           |
| =:tags=         | yes      | Tags list (first tag identifies journal notes)         |
| =:head=         | no       | Header content after #+title                           |
| =:body=         | no       | Note body content                                      |
| =:properties=   | no       | Alist for property drawer                              |
| =:meta=         | no       | Alist for metadata                                     |
| =:context=      | no       | Plist for template variables                           |

Can also be a function for dynamic configuration:
...
*Note on syntax:* =:file-name= and =:title= use strftime format (=%Y%m%d=) expanded for the target date. Other keys (=:head=, =:body=, etc.) use vulpea's =%<format>= syntax expanded at creation time.

*** Two-Pane Interface

The journal opens in a two-window layout:
- *Left*: Org-mode buffer for editing the daily note
- *Right*: Widgets buffer showing contextual information

Navigation keybindings in the widgets buffer:
...
Enable widgets by setting =vulpea-journal-widgets=:

#+begin_src emacs-lisp
;; Configure which widgets to display (in order)
...
Compact month calendar showing days with journal entries. Click any day to navigate.
...
Displays journal entries from the same date in previous years with content preview.
...
;; Handle leap year edge cases (Feb 29 / Mar 1)
(setq vulpea-journal-widget-previous-years-handle-leap-year t)
#+end_src

*** Calendar Integration

Vulpea integrates with Emacs calendar mode:
...
When enabled, dates with journal entries are highlighted in the calendar. Press =j= in calendar to open that day's journal.

*** Creating Custom Widgets

Define new widgets using the =vulpea-journal-define-widget= macro:
...
(vulpea-journal-define-widget my-tasks
  ...
  :order 25                              ; Display order (lower = earlier)
  :collapsible t                         ; Can be collapsed
  :default-collapsed nil                 ; Start expanded
  :query #'my-tasks-query                ; (date) -> list of items
  :render #'my-tasks-render              ; (item) -> propertized string
  ...
  (vulpea-db-query
   (lambda (note)
     (when-let ((deadline (vulpea-note-deadline note)))
       (time-equal-p (date-to-day date)
                     (date-to-day deadline))))))
...
|--------------------------------+------------------------------------|
| =vulpea-journal=                 | Open journal (today or given date) |
| =vulpea-journal-today=           | Open today's journal               |
| =vulpea-journal-note=            | Get or create note for date        |
| =vulpea-journal-find-note=       | Find existing note for date        |
| =vulpea-journal-note-p=          | Check if note is a journal entry   |
| =vulpea-journal-goto-date=       | Navigate to specific date          |
| =vulpea-journal-goto-next-day=   | Navigate to next day               |
| =vulpea-journal-goto-prev-day=   | Navigate to previous day           |
| =vulpea-journal-dates-in-month=  | Get journal dates in a month       |
| =vulpea-journal-dates-in-range=  | Get journal dates in date range    |
| =vulpea-journal-refresh-widgets= | Refresh widgets buffer             |
...
| =vulpea-utils-with-note=         | Execute body with buffer visiting note  |
| =vulpea-utils-with-note-sync=    | Execute body, save, and sync database   |

**** Modifying Notes Programmatically

Use =vulpea-utils-with-note-sync= when you need to modify a note and
immediately query the updated data:

#+begin_src emacs-lisp
;; Modify and sync - database is updated immediately
(vulpea-utils-with-note-sync event
  (vulpea-buffer-meta-set "status" "done" 'append))

;; Safe to query right away
(vulpea-meta-get event "status")  ; => "done"
#+end_src

For read-only operations or when async sync is acceptable, use
=vulpea-utils-with-note=:

#+begin_src emacs-lisp
;; Read-only - no sync needed
(vulpea-utils-with-note event
  (org-entry-get nil "CUSTOM_ID"))

;; Or when you'll trigger sync manually later
(vulpea-utils-with-note event
  (vulpea-buffer-meta-set "key" "value")
  (save-buffer))
;; Async sync will happen via file watcher
#+end_src
...
- =vulpea-journal.el= - Daily notes, two-pane interface, calendar integration
- =vulpea-journal-widget.el= - Widget system using ewoc
- =vulpea-journal-widgets.el= - Built-in widgets (calendar, created-today, links-to-today, previous-years)
