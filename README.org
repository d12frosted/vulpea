...
- *Stays out of your way* - file watchers feed a background queue, and a background worker process takes parsing off the main thread.
...
#+begin_src emacs-lisp
;; Parsing already runs in a background process by default; 'full
;; moves the database write there too, so saving a note - even a
...
| [[file:docs/comparison.org][Comparison]] | How vulpea compares to org-roam, org-node and Supertag |
