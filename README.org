...
:PROPERTIES:
:ID:                     8b14b943-22be-4654-ab52-79b5198bb44a
:END:

This function behaves like =vulpea-select-from=, but allows to select multiple notes at once. In order to stop iterating, use ~C-g~. It's important to note that it doesn't allow to select the same note more than once - already selected notes are removed from the prompt.

If you want to allow selecting the same note multiple times, you can use the following snippet:
...
- =vulpea-buffer-meta-get= - function to get a value of =PROP= from current buffer. Value is parsed based on the passed =TYPE= or as a string if omitted. Use it when you need to get a single meta =PROP= from current buffer, if you need to extract more, it's better to use =vulpea-buffer-meta-get!= for performance reasons.
- =vulpea-buffer-meta-get-list= function to get all values of =PROP= from current buffer. Values are parsed based on the passed =TYPE= or as a string if omitted. Use it when you need to get a single meta =PROP= from current buffer, if you need to extract more, it's better to use =vulpea-buffer-meta-get-list!= for performance reasons.
