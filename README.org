...
(let ((note (vulpea-db-get-by-id "wine-id-123")))
  ;; Get metadata with type coercion
...
;; Full scan of all configured directories
...
;; 'async: scan in background (may cause lag)
(setq vulpea-db-sync-scan-on-enable 'async)

;; 'blocking: wait for scan to complete
...
1. Disable heading-level indexing if not needed (2-3x speedup)
2. Use =fswatch= for external monitoring (more efficient than polling)
3. Increase batch size for bulk operations
4. Adjust debounce delay based on your workflow
...
- Database core: 11 tests ✅
- Parser & extractors: 18 tests ✅
- File watching & sync: 19 tests ✅
- Query layer: 30 tests ✅
