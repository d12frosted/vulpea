...
A full scan needs to run at least once to populate the database:

#+begin_src emacs-lisp
;; Interactive equivalent: M-x vulpea-db-sync-full
(vulpea-db-sync-full)
#+end_src

The first run parses every file (expect a few minutes for 100k+ notes).
Re-run it with a prefix argument (=C-u M-x vulpea-db-sync-full=) if you
change global settings and want to rebuild everything.
...
- =single-temp-buffer=: ⚡ ~1.3k files/sec. Skips org-mode hooks and per-file keywords.
- =temp-buffer=: ✓ Honors org-mode hooks per file. Slower if hooks are heavy.
- =find-file=: ✓ Respects dir-locals/file-visiting hooks. 30-40x slower than =single-temp-buffer=.

If an `org-mode-hook` is expensive (e.g. `org-roam`), wrap it in
`(unless (bound-and-true-p 'vulpea-db--active-parse-method) …)` so it is
skipped when Vulpea parses notes. See =bench/PERFORMANCE.md= for detailed benchmarks.
